...
function MatchCreateForm() {
    ...
    return <main className="flow-page"><div className="flow-shell"><Link href={`/facilities/${facilityId}`} className="flow-back">← 경기장으로 돌아가기</Link><div className="flow-heading"><span>STEP 1 OF 2</span><h1>함께할 매치를 만들어보세요</h1><p>모집 조건을 설정한 뒤 경기장 결제를 진행합니다.</p></div><div className="selected-slot"><div><span>선택한 경기 시간</span><b>{slotLabel}</b></div><strong>{amount.toLocaleString()}원</strong></div><div className="policy-notice"><b>환불/마감 정책</b><p>모집 마감은 경기 시작 1시간 전, 참가자 취소 가능 마감은 경기 시작 24시간 전으로 자동 설정됩니다.</p><ul><li>결제 실패·만료: 환불 없이 자리 선점만 취소</li><li>참가자 직접 이탈: 24시간 전까지 전액 환불</li><li>최소 인원 미달 자동 취소: 결제 완료 참가자 전원 전액 환불</li><li>방장 취소: 경기 시작 3일 전까지만 가능, 참가자 전원 전액 환불</li></ul></div><form className="flow-form" onSubmit={submit}><FormError message={error}/><Field label="매치 제목" htmlFor="title"><Input id="title" name="title" required maxLength={100} placeholder="예: 퇴근 후 가볍게 풋살 한 게임!"/></Field><div className="flow-grid"><Field label="종목" htmlFor="sportType"><Select id="sportType" name="sportType" defaultValue={initialSport}>{SPORTS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</Select></Field><Field label="모집 정원" htmlFor="capacity"><Input id="capacity" name="capacity" type="number" min={2} required defaultValue={10}/></Field><Field label="1인 참가비" htmlFor="feePerPerson"><Input id="feePerPerson" name="feePerPerson" type="number" min={0} required defaultValue={10000}/></Field><Field label="성별 조건" htmlFor="requiredGender"><Select id="requiredGender" name="requiredGender" defaultValue="ANY"><option value="ANY">성별 무관</option><option value="MALE">남성</option><option value="FEMALE">여성</option><option value="MIXED">혼성</option></Select></Field><Field label="최소 실력" htmlFor="minSkillLevel"><Select id="minSkillLevel" name="minSkillLevel" defaultValue="ANY">{LEVELS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</Select></Field><Field label="최대 실력" htmlFor="maxSkillLevel"><Select id="maxSkillLevel" name="maxSkillLevel" defaultValue="ANY">{LEVELS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</Select></Field><Field label="모집 마감" htmlFor="recruitDeadline"><Input id="recruitDeadline" name="recruitDeadline" type="datetime-local" required readOnly value={recruitDeadline ? toDateTimeLocalValue(recruitDeadline) : ""}/></Field><Field label="참가 취소 마감" htmlFor="cancelDeadline"><Input id="cancelDeadline" name="cancelDeadline" type="datetime-local" required readOnly value={cancelDeadline ? toDateTimeLocalValue(cancelDeadline) : ""}/></Field></div><p className="deadline-preview">경기 시작 {formatPolicyDateTime(matchStartAt)} 기준 · 모집 마감 {formatPolicyDateTime(recruitDeadline)} · 참가 취소 마감 {formatPolicyDateTime(cancelDeadline)}</p><Button loading={submitting} type="submit">매치 만들고 결제하기</Button></form></div></main>;
}

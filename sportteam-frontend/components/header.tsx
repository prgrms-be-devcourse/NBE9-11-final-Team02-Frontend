...
export function Header() {
  ...
  return <header className="site-header"><div className="container header-inner">
    ...
    <nav className={open ? "open" : ""}><Link href="/matches">매치 찾기</Link><Link href="/facilities">경기장</Link>{!loading && (user?.role === "MANAGER" ? <Link href="/manager/facilities">내 경기장</Link> : <Link href="/mypage">마이 플레이</Link>)}{!loading && (user ? <><Link href="/notifications">알림</Link><button className="logout-button" onClick={handleLogout}>로그아웃</button></> : <><Link className="login-link" href="/login">로그인</Link><Link className="signup-link" href="/signup">회원가입</Link></>)}</nav>
  </div></header>;
}

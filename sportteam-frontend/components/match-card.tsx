...
export function MatchCard({ match }: { match: MatchSummary }) {
  ...
  return <Link className="match-card" href={`/matches/${match.matchId}`}>
    ...
    <div className="match-body"><div className="match-tags"><span>{sport.label}</span><small>{match.minSkillLevel === "BEGINNER" ? "초보 환영" : "모든 레벨"}</small></div><h3>{match.title}</h3><dl><div><dt>일시</dt><dd>{date} · {time}</dd></div><div><dt>조건</dt><dd>{match.requiredGender === "ANY" ? "성별 무관" : match.requiredGender === "MALE" ? "남성" : "여성"} · {match.minSkillLevel}~{match.maxSkillLevel}</dd></div></dl><div className="match-bottom"><div className="people"><span/><span/><span/><b>{match.currentCount}/{match.capacity}명</b></div><strong>{match.feePerPerson.toLocaleString()}원</strong></div></div>
  </Link>;
}
